...
class BST {
 private:
    ...
    void loadFromFile(Node*& node, const std::string& filename) {
        ...
        while (file >> word) {
            ...
            if (!cleanWord.empty()) {
                if (!search(node, cleanWord)) {
                    node = insert(node, cleanWord);
                    size++;
                }
            }
        }
    ...
    bool search(Node* node, const std::string& value) const {
        if (!node) return false;
        if (value == node->data) return true;
        if (value < node->data) return search(node->left, value);
        return search(node->right, value);
